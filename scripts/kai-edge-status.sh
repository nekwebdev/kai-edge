  ...
  if [[ -r "$status_file" ]]; then
    if command -v jq >/dev/null 2>&1; then
      jq -r '
        ...
        "wake detections: " + ((.counters.wake_detections // 0) | tostring),
        "wake post accepted utterances: " + ((.counters.wake_post_accepted_utterances // 0) | tostring),
        "wake post speech timeouts: " + ((.counters.wake_post_speech_timeouts // 0) | tostring),
        "wake retrigger suppressions: " + ((.counters.wake_retrigger_suppressions // 0) | tostring),
