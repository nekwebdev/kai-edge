...
- `kai` rollout branch policy:
  ...
  - bootstrap reruns may proceed when `kai-local` is dirty only in `config.env`; in that case bootstrap should skip git branch sync and continue provisioning
  - bootstrap must still fail loudly when dirty paths include anything besides `config.env`
